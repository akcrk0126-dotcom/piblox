  ...
  보안 규칙 예시 (Realtime Database > 규칙):
  {
    "rules": {
      "playhub_mp": {
        ...
        "users": {
          ".read": "auth != null",
          ".indexOn": ["nicknameLower", "displayNameLower"],
          "$uid": {
...
const MP = (function () {
  ...
  function searchUsers(queryStr, cb) {
    ...
    const byField = (field) => db.ref(`${MP_ROOT}/users`)
      ...
      .catch(err => { lastErr = err; console.error('[MP] 검색 실패(' + field + ')', err); return []; });
    Promise.all([byField('nicknameLower'), byField('displayNameLower')]).then(([a, b]) => {
      ...
      // 결과가 없고 에러가 있었다면 원인을 같이 넘겨서 화면에 표시할 수 있게 한다
      cb && cb(arr, arr.length === 0 ? lastErr : null);
    }).catch(err => { console.error('[MP] 유저 검색 실패', err); cb && cb([], err); });
  }
      ...
      fUids.forEach(fid => {
        db.ref(`${MP_ROOT}/users/${fid}`).once('value').then(uSnap => {
          ...
          result[fid] = {
            ...
            nickname: uData.displayName || uData.nickname || friends[fid].name || '친구',
            accountName: uData.nickname || null,
